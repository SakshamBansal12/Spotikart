...
const { searchArtist1Tracks,searchArtist2Tracks,searchArtist3Tracks, getRecommendations }= require("./spotify/actions");
...
app.post("/recommendations", async (req, res) => {
  ...
  if(!artist1 || !artist2 || !artist3) {
    return res.status(400).send({ message: "Bad Request - must pass all three artists name " })
  }
  ...
  try {
    const result = await searchArtist1Tracks(http, { artist1 })
  ...
  try {
    const result = await searchArtist2Tracks(http, { artist2 })
    const { artists } = result
  ...
  try {
    const result = await searchArtist3Tracks(http, { artist3 })
    const { artists } = result
